...
print('-> lonely kernel')
init_kernel=kl.ExpSineSquared(1.0, 2.0, 2.0)
kl.likelihood(init_kernel, x2, x2, y2, yerr2)
start = time() # Calculation using george 
...
start = time() # Calculation using george 
kernel = 1.**2*ExpSquaredKernel(1.0**2) * ExpSine2Kernel(2.0/1.0**2, 1.0) +  WhiteKernel(1.0)
gp = george.GP(kernel)
gp.compute(x,yerr)
print 'Took %f seconds' % (time() - start), ('log_p_george',gp.lnlikelihood(y))
